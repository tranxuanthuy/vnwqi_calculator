from units.fomula import first_fomula, second_fomula
'''GROUP I: pH'''
def wqi_ph(value):
...
def wqi_aldrin(value, threshold=0.1):
...
def wqi_bhc(value, threshold=0.02):
...
def wqi_dieldrin(value, threshold=0.1):
...
def wqi_ddt(value, threshold=1.0):
...
def wqi_heptachlor_heptachlorepoxide(value, threshold=0.2):
...
def wqi_as(value):
    ...
    if value <= bqi[0]:
        ...
        return 10
    elif value > bqi[0] and value <= bqi[1]:
        return first_fomula(qi[0], qi[1], bqi[0], bqi[1], value)
    elif value > bqi[1] and value <= bqi[2]:
        return first_fomula(qi[1], qi[2], bqi[1], bqi[2], value)
    elif value > bqi[2] and value <= bqi[3]:
        return first_fomula(qi[2], qi[3], bqi[2], bqi[3], value)
