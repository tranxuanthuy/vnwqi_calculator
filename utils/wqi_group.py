from utils.tables import group_definitions
def wqi_group_1(params_data: dict) -> float | None:
    from utils.wqi_si import wqi_ph
    if "pH" in params_data:
        return wqi_ph(params_data["pH"])
    return None

def wqi_group_2(params_data: dict) -> float | None:
    from utils.wqi_si import wqi_aldrin, wqi_bhc, wqi_dieldrin, wqi_ddts, wqi_heptachlor_heptachlorepoxide
    import math
...
def wqi_group_3(params_data: dict) -> float | None:
    from utils.wqi_si import wqi_as, wqi_cd, wqi_pb, wqi_cr6, wqi_cu, wqi_zn, wqi_hg
    import math
...
def wqi_group_4(params_data: dict) -> float | None:
    from utils.wqi_si import wqi_do, wqi_bod5, wqi_cod, wqi_toc, wqi_nnh4, wqi_nno3, wqi_nno2, wqi_ppo4
...
def wqi_group_5(params_data: dict) -> float | None:
    from utils.wqi_si import wqi_coliform, wqi_ecoli
